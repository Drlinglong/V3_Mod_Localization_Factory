...
const mockGlossaryData = {
    'v3-art': [
        { id: 'v3_art_001', source: 'concept_art', translation: '概念艺术', notes: '美术专有名词', variants: ['观念艺术'] },
        { id: 'v3_art_002', source: 'impressionism', translation: '印象主义', notes: '', variants: [] },
    ],
    'v3-eco': [
        { id: 'v3_eco_001', source: 'gdp', translation: '国内生产总值', notes: '宏观经济学指标', variants: ['GDP'] },
        { id: 'v3_eco_002', source: 'market_price', translation: '市场价格', notes: '', variants: [] },
    ],
    's-ships': [
        { id: 's_ships_001', source: 'corvette', translation: '护卫舰', notes: '小型舰船', variants: ['小型护卫舰'] },
        { id: 's_ships_002', source: 'destroyer', translation: '驱逐舰', notes: '中型舰船', variants: [] },
    ],
    's-events': [{ id: 's_events_001', source: 'anomaly', translation: '异常', notes: '', variants: [] }],
    's-overlord': [{ id: 's_overlord_001', source: 'holding', translation: '附属建筑', notes: '行星总督的特殊建筑', variants: [] }],
};
...
const GlossaryManagerPage = () => {
    ...
    const handleModalOk = () => {
        form.validateFields().then(values => {
            ...
            let updatedData;
            if (editingEntry) {
                updatedData = data.map(item => item.id === editingEntry.id ? { ...item, ...processedValues } : item);
            } else {
                const newEntry = { id: `new_${Date.now()}`, ...processedValues };
    ...
    const columns = [
        ...
        { accessorKey: 'translation', header: () => t('glossary_translation'), cell: info => info.getValue() },
    ...
    return (
        <div style={{ padding: '24px' }}>
            ...
            <Modal
                title={editingEntry ? t('glossary_edit_entry') : t('glossary_add_entry')}
                open={isModalVisible}
                onOk={handleModalOk}
                onCancel={() => setIsModalVisible(false)}
                destroyOnClose
            >
                <Form form={form} layout="vertical" name="glossary_entry_form" initialValues={{ variants: '' }}>
                    <Form.Item name="source" label={t('glossary_source_text')} rules={[{ required: true }]}>
                        <Input />
                    </Form.Item>
                    <Form.Item name="translation" label={t('glossary_translation')} rules={[{ required: true }]}>
                        <Input />
                    </Form.Item>
                    <Form.Item name="notes" label={t('glossary_notes')}>
                        <Input.TextArea />
                    </Form.Item>
                    <Form.Item name="variants" label={t('glossary_variants')} tooltip={t('glossary_variants_tooltip')}>
                        <Input placeholder={t('glossary_variants_placeholder')} />
                    </Form.Item>
                </Form>
            </Modal>
        </div>
