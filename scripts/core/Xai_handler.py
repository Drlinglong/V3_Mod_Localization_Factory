...
def initialize_client(api_key: str = None) -> "OpenAI | None":
    """Initializes the Grok client via Azure AI Foundry."""
    if not api_key and not os.getenv("GROK_API_KEY"):
        logging.error("Grok API Key not found in environment variables.")
        return None
    try:
        # 使用Azure AI Foundry的Grok-4-Fast-Reasoning模型
        client = OpenAI(
            api_key=api_key or os.getenv("GROK_API_KEY"),
            base_url=os.getenv("GROK_BASE_URL", "https://your-azure-endpoint.com")
        )
        model_name = API_PROVIDERS["grok"]["default_model"]
        logging.info(f"Grok client initialized successfully, using model: {model_name}")
        return client
    except Exception as e:
        logging.exception(f"Error initializing Grok client: {e}")
        return None
