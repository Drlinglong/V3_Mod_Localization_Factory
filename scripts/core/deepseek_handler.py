...
def initialize_client(api_key: str = None) -> "OpenAI | None":
    ...
    if not api_key and not os.getenv("DEEPSEEK_API_KEY"):
        ...
        return None
    try:
        # 使用DeepSeek-V3.2-Exp (Non-thinking Mode)
        client = OpenAI(
            api_key=api_key or os.getenv("DEEPSEEK_API_KEY"),
            base_url="https://api.deepseek.com"
        )
        model_name = API_PROVIDERS["deepseek"]["default_model"]
        logging.info(f"DeepSeek client initialized successfully, using model: {model_name}")
        return client
    except Exception as e:
        logging.exception(f"Error initializing DeepSeek client: {e}")
        return None
